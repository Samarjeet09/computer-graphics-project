...
// custom function to load a BMP image
...
void disp_building() // Building block interiors
{
    int k;
    glPushMatrix();
...
public:
    building(float a, float b, float c, float red = 0.502, float green = 0.01, float blue = 0.01)
    {
        structure[0][0] = 0;
        structure[0][1] = 0;
        structure[0][2] = 0;
...
#pragma warning(disable : 4309)
    void disp_build(const char text[15], char side = '/0')
#pragma warning(pop)
    {
        float door[3] = { 0.0f, 0.0f, 0.0f };
        ...
        {
            //draw windows on front side of building
...
void changeSize(int w, int h)
{
    ...
    gluPerspective(69.0, ratio, 0.1, 200.0); // perspective transformation
...
void disp_roads() // display the roads in the campus
{
    ...
    // Add white lines on the roads
    glColor3f(1, 1, 1);
    glLineWidth(2.0);
    glBegin(GL_LINES);
    for (float i = -40; i <= -20; i += 5)
    {
        glVertex3f(i, 0.11, 90);
        glVertex3f(i, 0.11, -70);
    }
    for (float i = -20; i <= 90; i += 5)
    {
        glVertex3f(i, 0.11, 55);
        glVertex3f(i, 0.11, 60);
    }
    for (float i = -20; i <= 40; i += 5)
    {
        glVertex3f(i, 0.11, 75);
        glVertex3f(i, 0.11, 80);
    }
    for (float i = 35; i <= 40; i += 5)
    {
        glVertex3f(i, 0.11, 75);
        glVertex3f(i, 0.11, -70);
    }
...
void trees() // draw a tree
{
    ...
    glPushMatrix();
    float k = 0;
    for (int i = 0, j = 0; i < 4; i++, j += 0.4, k += 0.15)
    {
...
void draw_arch(const char text[10]) // draw the arch
{
...
void display()
{
    // sky blue background set as default
    ...
    gluLookAt(
        ...
        0.0, 1.0, 0.0);
    // printf("\nz=%f\tx=%f\n", z, x);
    // make ground using quads
    glColor3f(0, 0.87, 0.3);
    glBegin(GL_QUADS);
    ...
    mech.disp_build("SPORTS CLUB", 'r');
    glPopMatrix();
    mech_court();
    glPushMatrix();
    glTranslatef(-70, 0, -30);
    coscomplex.disp_build("COS", 'r');
    glPopMatrix();
    disp_building();
    glPushMatrix();
